...
# ==================== 差分进化算法实现 ====================
class 差分进化算法:
    def __init__(self, 目标函数, 种群大小=50, 最大迭代次数=1000, 维度=10, 范围=(-100, 100),
                 变异因子=0.8, 交叉率=0.7, 变异策略='rand/1', 收敛阈值=1e-8):
        self.目标函数 = 目标函数
        self.种群大小 = 种群大小
        self.最大迭代次数 = 最大迭代次数
        self.维度 = 维度
...
def 批量测试CEC2005函数(算法参数=None):
    ...
    if not os.path.exists('DE的CEC2005测试结果'):
        os.makedirs('DE的CEC2005测试结果')

    if not os.path.exists('DE的CEC2005测试结果/收敛曲线'):
        os.makedirs('DE的CEC2005测试结果/收敛曲线')

    所有结果 = {}
    详细结果 = {}

    print("开始批量测试DE的CEC2005函数（遵循CEC 2005评估标准）...")
    ...
    for 函数名 in CEC2005函数列表:
        ...
        for 维度 in 支持维度列表:
            ...
            for 运行编号 in range(运行次数):
                ...
                最优解, 最优值, 收敛代数, 收敛时间, 总FES = 差分进化算法实例.运行优化(理论最优值)
                ...
                if hasattr(差分进化算法实例, '函数评估次数记录'):
                    FES记录 = 差分进化算法实例.函数评估次数记录
                    最优值记录 = 差分进化算法实例.历史最优值
...
def 生成CEC标准报告(所有结果, 详细结果):
    ...
    汇总报告.to_excel('DE的CEC2005测试结果/DE的CEC2005_详细报告.xlsx', index=False)
    汇总报告.to_csv('DE的CEC2005测试结果/DE的CEC2005_详细报告.csv', index=False, encoding='utf-8-sig')

    with open('DE的CEC2005测试结果/DE的CEC2005_原始数据.json', 'w', encoding='utf-8') as f:
...
def 绘制CEC收敛曲线(详细结果, 选择函数=None, 选择维度=10):
    ...
    plt.tight_layout()
    plt.savefig('DE的CEC2005测试结果/收敛曲线/误差分布对比.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def 绘制性能对比雷达图(所有结果, 维度=10):
    ...
    plt.title(f'DE的CEC2005函数在{维度}维下的性能对比雷达图', size=16, color='blue', y=1.1)

    ax.legend(['归一化平均误差'], loc='upper right', bbox_to_anchor=(0.1, 0.1))

    plt.tight_layout()
    plt.savefig(f'DE的CEC2005测试结果/维度{维度}性能雷达图.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def 绘制维度影响分析(所有结果, 选择函数='F12005'):
    ...
    plt.tight_layout()
    plt.savefig(f'DE的CEC2005测试结果/{选择函数}维度影响分析.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def 绘制函数类型对比(所有结果, 维度=10):
    ...
    plt.tight_layout()
    plt.savefig(f'DE的CEC2005测试结果/维度{维度}函数类型对比.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def 生成性能排名表(所有结果, 维度=10):
    ...
    # 保存排名表
    排名表.to_excel(f'DE的CEC2005测试结果/维度{维度}性能排名.xlsx', index=False)
    排名表.to_csv(f'DE的CEC2005测试结果/维度{维度}性能排名.csv', index=False, encoding='utf-8-sig')
...
if __name__ == "__main__":
    ...
    print(f"\n所有结果已保存到 'DE的CEC2005测试结果' 目录中")
    print(f"测试报告: DE的CEC2005测试结果/DE的CEC2005_详细报告.xlsx")
    print(f"原始数据: DE的CEC2005测试结果/DE的CEC2005_原始数据.json")
