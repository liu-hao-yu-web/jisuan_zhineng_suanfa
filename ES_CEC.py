...
class 进化策略算法:
    ...
    def 运行优化(self, 理论最优值=None):
        ...
        for 迭代次数 in range(self.最大迭代次数):
            ...
            self.种群位置, self.变异强度, self.种群适应度 = self.选择操作(子代位置, 子代变异强度)
    ...
    for 函数名 in CEC2005函数列表:
        ...
        for 维度 in 支持维度列表:
            ...
            for 运行编号 in range(运行次数):
                ...
                最优解, 最优值, 收敛代数, 收敛时间, 总FES = 进化策略算法实例.运行优化(理论最优值)
                ...
                if hasattr(进化策略算法实例, '函数评估次数记录') and hasattr(进化策略算法实例, '历史最优值记录'):
                    FES记录 = 进化策略算法实例.函数评估次数记录
                    最优值记录 = 进化策略算法实例.历史最优值记录
