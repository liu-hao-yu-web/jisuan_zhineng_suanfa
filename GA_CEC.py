...
#==================== 遗传算法优化器 ====================
class 遗传算法优化器:
    def __init__(self, 目标函数, 种群大小=50, 最大迭代次数=1000, 维度=10, 变量范围=(-100, 100),
                 交叉率=0.8, 变异率=0.1, 收敛阈值=1e-8):
        self.目标函数 = 目标函数
        self.种群大小 = 种群大小
        self.最大迭代次数 = 最大迭代次数
        self.维度 = 维度
    ...
    def 初始化种群(self):
        ...
        self.种群位置 = np.random.uniform(
            self.变量范围[0], self.变量范围[1], (self.种群大小, self.维度))

        
        self.种群适应度 = np.array([self.目标函数(个体) for 个体 in self.种群位置])
        self.总函数评估次数 += self.种群大小
    ...
    def 选择操作(self):
        ...
        for _ in range(self.种群大小):
            ...
            候选索引 = np.random.choice(range(self.种群大小), size=3, replace=False)
            最优候选索引 = 候选索引[np.argmin(self.种群适应度[候选索引])]

            新种群位置.append(self.种群位置[最优候选索引])
            新种群适应度.append(self.种群适应度[最优候选索引])

        self.种群位置 = np.array(新种群位置)
        self.种群适应度 = np.array(新种群适应度)

    def 交叉操作(self):
        
        新种群 = []
        交叉标记 = np.random.rand(self.种群大小 // 2) < self.交叉率
            ...
            if 交叉标记[i // 2]:
                ...
                for j in range(self.维度):
                    if np.random.rand() <= 0.5:
                        if abs(父代1[j] - 父代2[j]) > 1e-10:
                            if 父代1[j] < 父代2[j]:
                                y1, y2 = 父代1[j], 父代2[j]
                            else:
                                y1, y2 = 父代2[j], 父代1[j]

                            β = 1.0 + 2.0 * min((y1 - self.变量范围[0]), (self.变量范围[1] - y2)) / (y2 - y1)
                            α = 2.0 - β ** -(分布指数 + 1.0)
                            ...
                            if u <= 1.0 / α:
                                ...
                                βq = (1.0 / (2.0 - u * α)) ** (1.0 / (分布指数 + 1.0))

                            子代1[j] = 0.5 * ((y1 + y2) - βq * (y2 - y1))
                            子代2[j] = 0.5 * ((y1 + y2) + βq * (y2 - y1))
                        else:
                            子代1[j] = 父代1[j]
                            子代2[j] = 父代2[j]
                    else:
                ...
                子代1 = np.clip(子代1, self.变量范围[0], self.变量范围[1])
                子代2 = np.clip(子代2, self.变量范围[0], self.变量范围[1])
    ...
    def 变异操作(self):
        ...
        变异标记 = np.random.rand(self.种群大小, self.维度) < self.变异率
        分布指数 = 20
        ...
        for i in range(self.种群大小):
            for j in range(self.维度):
                if 变异标记[i, j]:
                    ...
                    if u < 0.5:
                        δq = (2.0 * u + (1.0 - 2.0 * u) *
                              (1.0 - δ) ** (分布指数 + 1.0)) ** (1.0 / (分布指数 + 1.0)) - 1.0
                    else:
                        δq = 1.0 - (2.0 * (1.0 - u) + 2.0 * (u - 0.5) *
                                    δ ** (分布指数 + 1.0)) ** (1.0 / (分布指数 + 1.0))

                    self.种群位置[i, j] = y + δq * (yu - yl)

            
            self.种群位置[i] = np.clip(self.种群位置[i], self.变量范围[0], self.变量范围[1])
    ...
    def 检查收敛(self, 当前代数):
        ...
        当前误差 = abs(self.全局最优值 - getattr(self, '理论最优值', 0))

        if 当前误差 < self.收敛阈值:
            if self.收敛代数 == self.最大迭代次数:  
                self.收敛代数 = 当前代数
    ...
    def 运行优化(self, 理论最优值=None):
        ...
        self.历史平均适应度.append(np.mean(self.种群适应度))
        self.函数评估次数记录.append(self.总函数评估次数)
        ...
        for 迭代次数 in range(self.最大迭代次数):
            ...
            if self.检查收敛(迭代次数):
                if 迭代次数 % 100 == 0:
                    print(f"迭代次数: {迭代次数}, 当前最优值: {self.全局最优值:.6e}, "
                          f"当前误差: {abs(self.全局最优值 - self.理论最优值):.2e}")
        ...
        print(f"优化完成! 最优值: {self.全局最优值:.6e}, "
              f"收敛代数: {self.收敛代数}, 总FES: {self.总函数评估次数}")

        return (self.全局最优解, self.全局最优值,
                self.收敛代数, self.收敛时间, self.总函数评估次数)
...
def 批量测试CEC2005函数(算法参数=None):
    ...
    if not os.path.exists('GA的CEC2005测试结果'):
        os.makedirs('GA的CEC2005测试结果')

    if not os.path.exists('GA的CEC2005测试结果/收敛曲线'):
        os.makedirs('GA的CEC2005测试结果/收敛曲线')

    所有结果 = {}
    详细结果 = {}

    print("开始批量测试GA的CEC2005函数（遵循CEC 2005评估标准）...")
    ...
    for 函数名 in CEC2005函数列表:
        ...
        for 维度 in 支持维度列表:
            ...
            for 运行编号 in range(运行次数):
                ...
                最优解, 最优值, 收敛代数, 收敛时间, 总FES = 遗传算法实例.运行优化(理论最优值)
                ...
                if len(遗传算法实例.函数评估次数记录) > 0:
                    
                    FES1000误差列表.append(真实误差)  
                    FES10000误差列表.append(真实误差)  
                    FES100000误差列表.append(真实误差)  
            ...
            排序后FES = np.sort(函数评估次数列表) if 达到精度的FES列表 else []

            
            统计结果 = {
                '理论最优值': 理论最优值,
                '目标精度': 目标精度,
                '平均误差': float(np.mean([e for e in 最终误差列表 if e is not None])) if 最终误差列表 else 0.0,
                '误差标准差': float(np.std([e for e in 最终误差列表 if e is not None])) if 最终误差列表 else 0.0,
                '最佳误差': float(np.min([e for e in 最终误差列表 if e is not None])) if 最终误差列表 else 0.0,
                '最差误差': float(np.max([e for e in 最终误差列表 if e is not None])) if 最终误差列表 else 0.0,
                '平均收敛时间': float(np.mean([t for t in 收敛时间列表 if t is not None])) if 收敛时间列表 else 0.0,
                '平均收敛代数': float(np.mean([g for g in 收敛代数列表 if g is not None])) if 收敛代数列表 else 0.0,
                '平均FES': float(np.mean([f for f in 函数评估次数列表 if f is not None])) if 函数评估次数列表 else 0.0,
                '成功运行次数': 成功运行次数,
                '成功率': 成功运行次数 / 运行次数,
                '排序后误差': 排序后误差.tolist(),
                '排序后FES': 排序后FES.tolist() if 达到精度的FES列表 else [],
...
def 生成CEC标准报告(所有结果, 详细结果):
    ...
    汇总报告.to_excel('GA的CEC2005测试结果/GA的CEC2005_详细报告.xlsx', index=False)
    汇总报告.to_csv('GA的CEC2005测试结果/GA的CEC2005_详细报告.csv', index=False, encoding='utf-8-sig')

    
    with open('GA的CEC2005测试结果/GA的CEC2005_原始数据.json', 'w', encoding='utf-8') as f:
...
def 绘制CEC收敛曲线(详细结果, 选择函数=None, 选择维度=30):
    ...
    plt.tight_layout()
    plt.savefig('GA的CEC2005测试结果/收敛曲线/误差分布对比.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def 绘制性能对比雷达图(所有结果, 维度=10):
    ...
    plt.title(f'CEC2005函数在{维度}维下的性能对比雷达图', size=16, color='blue', y=1.1)

    
    ax.legend(['归一化平均误差'], loc='upper right', bbox_to_anchor=(0.1, 0.1))

    plt.tight_layout()
    plt.savefig(f'GA的CEC2005测试结果/维度{维度}性能雷达图.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def 绘制维度影响分析(所有结果, 选择函数='F12005'):
    ...
    plt.tight_layout()
    plt.savefig(f'GA的CEC2005测试结果/{选择函数}维度影响分析.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def 绘制函数类型对比(所有结果, 维度=10):
    ...
    plt.tight_layout()
    plt.savefig(f'GA的CEC2005测试结果/维度{维度}函数类型对比.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def 生成性能排名表(所有结果, 维度=10):
    ...
    排名表.to_excel(f'GA的CEC2005测试结果/维度{维度}性能排名.xlsx', index=False)
    排名表.to_csv(f'GA的CEC2005测试结果/维度{维度}性能排名.csv', index=False, encoding='utf-8-sig')
...
if __name__ == "__main__":
    ...
    print(f"\n所有结果已保存到 'GA的CEC2005测试结果' 目录中")
    print(f"测试报告: GA的CEC2005测试结果/GA的CEC2005_详细报告.xlsx")
    print(f"原始数据: GA的CEC2005测试结果/GA的CEC2005_原始数据.json")